...
function displayObjectBirdMoveTween(bird, moves, currentMove, startX, startY) {
  var tween = new TWEEN.Tween( { x: startX, y: startY  } )
    ...
    .onUpdate( function () {
        if (bird.state.queue.length === 0) {
          // animate spine object while tween animation is running
...
function displayObjectBirdAdd(scene) {
  ...
  var bird = new PIXI.Spine("../data/display/gfx/packed/spinebird.json");
  ...
  var bird2 = new PIXI.Spine("../data/display/gfx/packed/spinebird.json");

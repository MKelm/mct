...
mct.pixi = {
  ...
  currentScene : "gameNew", // change to start with a specific scene on start for development purposes
...
//mct.pixi.text.styles = baseLoadJSON('./lib/data/display/txt/styles.json');
// ^ does not work anylonger, styles are loaded in DisplayObjects depending on element object
//mct.pixi.text.lang = baseLoadJSON('./lib/data/display/txt/lngs/' + mct.status.lang + '.json');
// ^ does not work anylonger, translations are loaded in DisplayObjects depending on element object

// add all assets here!
mct.pixi.assetsToLoad = [
...
$(document).ready(function() {
  ...
  function displayInit() {
    ...
    style = {font: Math.floor(15 * mct.pixi.screen.ratio) + "px " + "Arial", fill: "FFFFFF"};
...
function displayInitScreenResize() {
  if (mct.pixi.screen.ratio !== 0) {
    ...
    mct.pixi.screen.width = mct.pixi.screen.originalSize.width;
    mct.pixi.screen.height = mct.pixi.screen.originalSize.height;
  } else {
    mct.pixi.screen.originalSize = { width: mct.pixi.screen.width, height: mct.pixi.screen.height };
  }

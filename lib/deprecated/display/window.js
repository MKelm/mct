...
function displayWindowLayerElements(
  ...
  if (elementsType == "text") {
    ...
    if (elements.length > 0) {
      ...
      var storage = [textLineLength[0]], stCount = 0;
      var maxColumnWidth = [textLineLength[0]];

      for (var i = 0; i < textLineLength[1]; i++) {
        if (text !== null) {
          elementPosition.y += text.height;
        }
        for (var j = 0; j < textLineLength[0]; j++) {
          text = displayWindowLayerText(
            location, align, elementPosition, padding, textStyle, null, elements[stCount][1], windowName
          );
          if (typeof storage[j] != "object") {
            storage[j] = [];
          }
          storage[j].push(text);
          if (typeof maxColumnWidth[j] != "number") {
            maxColumnWidth[j] = 0;
          }
          if (maxColumnWidth[j] < text.width) {
            maxColumnWidth[j] = text.width + text.position.x;
          }
          stCount++;
          if (stCount == elements.length) {
            break;
          }
        }
        if (stCount == elements.length) {
      ...
      stCount = 0;
      for (var i = 0; i < textLineLength[1]; i++) {
        elementPosition.x = storage[0][0].position.x;
        for (var j = 0; j < textLineLength[0]; j++) {
          if (j > 0) {
            elementPosition.x += maxColumnWidth[j-1];
          }
          storage[j][i].position.x = elementPosition.x;
          container.addChild(storage[j][i]);
          stCount++;
          if (stCount == elements.length) {
            break;
          }
...
function displayWindowLayerElementClick(mouseData) {
  //audioPlayEffect("ef.click2"); deprecated, use new AudioHandler!
...
function displayWindowAdd(name, json, texts, elements) {
  ...
  mct.pixi.windows[name] = {
    json : baseLoadJSON("./lib/data/display/window/" + json),
    layers : [],

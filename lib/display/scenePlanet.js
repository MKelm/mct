...
function displayScenePlanetAdd() {
  ...
  for (var fp in mct.game.data.technologies.fields) {
    if (mct.game.data.technologies.fields[fp].companies.for == 'type') {
      technologyFieldsForTypeElements.push([fp, mct.game.data.technologies.fields[fp].title]);
      technologyFieldTypesElements[fp] = [];
      for (var tp in mct.game.data.technologies.fields[fp].types) {
        technologyFieldTypesElements[fp].push([tp, mct.game.data.technologies.fields[fp].types[tp].title]);
      }
    } else {
      technologyFieldsForTypesElements.push([fp, mct.game.data.technologies.fields[fp].title]);
    }
  }
  console.log(technologyFieldTypesElements);
  displayWindowAdd(
    'planetcompanies',
    'scenes/windows/planet/companies.json',
    {}, // dynamic texts
    {

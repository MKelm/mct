...
function displayScenePlanetAdd() {
  ...
  var gridScale = 0.95;
  var startPoint = {
    x : Math.floor(planet.position.x - (planet.width*gridScale)/2),
    y : Math.floor(planet.position.y - (planet.height*gridScale)/2)
  };
  var nodeScaleX = (planet.width*gridScale) / mct.data.planets[mct.status.planet].object.units;
  var nodeScaleY = (planet.height*gridScale) / mct.data.planets[mct.status.planet].object.units;

  var nodes = mct.data.planets[mct.status.planet].object.grid.nodes;
  var nodeGfx = null;

  nodeGfx = new PIXI.Graphics();
    nodeGfx.position.x = planet.position.x - planet.width/2;
    nodeGfx.position.y = planet.position.y - planet.height/2;
  ...
  for (var i = 0; i < nodes.length; i++) {
    nodeGfx = new PIXI.Graphics();
    nodeGfx.position.x = startPoint.x + (nodes[i].x * nodeScaleX);
    nodeGfx.position.y = startPoint.y + (nodes[i].y * nodeScaleY);

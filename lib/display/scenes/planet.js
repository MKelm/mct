...
function displayScenePlanetAdd() {
  ...
  for (var i = 0; i < nodes.length; i++) {
    nodeGfx = new PIXI.Graphics();
    nodeGfx.position.x = (planet.position.x - (planet.width*gridScale/2)) + (nodes[i].x * nodeScaleX);
    nodeGfx.position.y = (planet.position.y - (planet.height*gridScale/2)) + (nodes[i].y * nodeScaleY);

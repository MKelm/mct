    ...
    if (typeof currentLayerData.contents != "undefined" && currentLayerData.contents.length > 0) {
      ...
      for (var i = 0; i < currentLayerData.contents.length; i++) {
        ...
        var lastContent = displayWindowLayerElements(
          ...
          content.padding,
          content.textStyle,
          (typeof content.textLineLength != "undefined") ? content.textLineLength : content.columns,
          (typeof content.text != "undefined") ? content.text : content.elements,
          (typeof content.columns != "undefined") ? "list" : 'text',
...
function displayWindowLayerElements(
  ...
  if (elementsType == "text") {
    ...
    // just a fixed value for the current layer status to get some contents,
    // needs a real layer status values detection later
    var layerStatus = ["advertisements"];
  ...
  if (typeof targetLayer != "undefined" && targetLayer > 0 &&
    ...
    if (mct.pixi.windows[windowName].layerInteractions[targetLayer].status.current != actionStatusHandle) {
      ...
      if (mct.pixi.windows[windowName].layerInteractions[sourceLayer].status.current !== "") {
        ...
        if (typeof mct.pixi.windows[windowName].layerInteractions[sourceLayer].status[actionStatusKey] != "undefined") {
      ...
      var exists = false;
      if (actionStatusChain.length > 0) {
        for (var p = 0; p < actionStatusChain.length; p++) {
          if (actionStatusChain[p][0] == targetLayer) {
            exists = true;
            actionStatusChain[p] = [targetLayer, parameter];
            break;
          }
        }
      }
      if (exists === false) {
        actionStatusChain.push([targetLayer, parameter]);
      }

...
function displayWindowAdd(name, json, texts, elements) {
  ...
  for (var i = 0; i < mct.pixi.windows[name].json.layers.length; i++) {
    if (typeof mct.pixi.windows[name].json.layers[i].visible != "undefined") {
      displayWindowLayer(name, "default", i)
    }
  }
}
...
function displayWindowLayer(windowName, parameter, targetLayer) {
  mct.pixi.windows[windowName].layers.push(
    displayWindowLayerContainer(windowName, mct.pixi.windows[windowName].json.layers[targetLayer])
  );

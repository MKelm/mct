...
function displayWindowLayerContainer(windowName, currentLayerData) {
    ...
    if (typeof currentLayerData.menus != "undefined" && currentLayerData.menus.length > 0) {
      ...
      mct.pixi.windows[windowName].tmp = { width : 0, height : 0 }; // temp values of last menu size
      ...
      for (var i = 0; i < currentLayerData.menus.length; i++) {
        ...
        if (lastLocation != "" && lastLocation != menu.location) {
          ...
          mct.pixi.windows[windowName].tmp = { width : 0, height : 0 };
        }
        if (menu.location == "top") {
          if (mct.pixi.windows[windowName].tmp.height == 0 && menuPosition.y == 0) {
            ...
            menuPosition.y = menuPosition.y + mct.pixi.windows[windowName].tmp.height;
          }
        } else {
          if (mct.pixi.windows[windowName].tmp.height == 0 && menuPosition.y == 0) {
        ...
        mct.pixi.windows[windowName].tmp.height += lastMenu.children[0].height;
        // add layer menu
...
function displayWindowAdd(name, json, texts, elements) {
  ...
  for (var i = 0; i < mct.pixi.windows[name].json.layers.length; i++) {
    ...
    if (mct.pixi.windows[name].json.layers[i].hidden != true) {
...
function displayWindowLayerByButton(windowName, targetLayer, parameter) {
  alert(windowName + " " + targetLayer + " " + parameter);
}

function displayWindowClose(windowName, parameter) {
  alert(windowName + " " + parameter);
}

...
function displayWindowLayerContainer(windowName, currentLayer, currentLayerData) {
    ...
    if (typeof currentLayerData.contents != "undefined" && currentLayerData.contents.length > 0) {
      var contentSpace = {
        ...
        bottom : mct.pixi.windows[windowName].layerCovers[currentLayer].bottom + layerTopLeftY + layerBorderSize
      };
      ...
      for (var i = 0; i < currentLayerData.contents.length; i++) {
        ...
        if (content.location == "top") {
          if (contentPosition.y == 0) {
            contentPosition.y = contentSpace.top;
          } else {
            contentPosition.y = lastContent.position.y + lastContent.children[0].position.y +
              lastContent.children[0].height;
          }
        } else {
          contentPosition.y = contentPosition.y - contentSpace.bottom;
        }
...
function displayWindowLayerElements(location, align, position, padding, textStyle, textLineLength, elements, elementsType, windowName, currentLayer) {

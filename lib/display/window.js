...
function displayWindowLayerContainer(windowName, currentLayer, currentLayerData, position) {
    ...
    if (typeof position != "object" || typeof position.x == "undefined") {
      ...
      var layerTopLeftX = position.x;
      var layerTopLeftY = position.y;
    }
...
function displayWindowAdd(name, json, texts, elements) {
  ...
  for (var i = 0; i < mct.pixi.windows[name].json.layers.length; i++) {
    if (typeof mct.pixi.windows[name].json.layers[i].visible != "undefined") {
  ...
  if (perform == true) {
    ...
    mct.pixi.windows[windowName].layers.push(
      displayWindowLayerContainer(
        windowName, targetLayer, mct.pixi.windows[windowName].json.layers[targetLayer], position
      )
    );

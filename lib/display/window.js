...
function displayWindowLayerTextSpecial(text) {
  if (text == "currentPlanet") {
    ...
    var name = mct.game.planets[mct.game.status.planet].name[
      mct.game.planets[mct.game.status.planet].storyVersion
    ];
    if (typeof name == "string") {
      return name;
    } else {
      return name[mct.game.status.lang];
    }
  }
  return "";
}
...
function displayWindowLayerMenu(location, align, position, padding, textStyle, buttons, windowName, currentLayer) {
  ...
  for (var i = 0; i < buttons.length; i++) {
    if (typeof buttons[i].elements != "undefined") {
      ...
      if (mct.pixi.windows[windowName].elements[buttons[i].elements].length > 0) {
      ...
      } else if (typeof buttons[i].targetLayer != "undefined") {
        ...
        if (mct.pixi.windows[windowName].elements[buttons[i].elements][currentActionParameterByParentLayerInteraction].length > 0) {
          for (var j = 0; j < mct.pixi.windows[windowName].elements[buttons[i].elements][currentActionParameterByParentLayerInteraction].length; j++) {
            elements = mct.pixi.windows[windowName].elements[buttons[i].elements][currentActionParameterByParentLayerInteraction];
            button = {
              text: elements[j][1],
              targetAction: buttons[i].action,
              actionParameter: elements[j][0]

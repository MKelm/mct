...
function displayChangeToggleSurvey(id, url) {
  if (mct.status.pixi.survey[id] == true) {
    ...
    $("#"+id).load(function (){
      ...
      // set version time and version hash to validate version for debugging!!
      if (id == "surveyBugs") {
              ...
              if (file != '.git') {
                ...
                fs.stat(file, function(err, stat) {
                  if (stat && stat.isDirectory()) {
                    ...
                    s.on('end', function() {
                      tmp = String(stat.mtime).substring(4, 24).replaceArray(monthIn, monthOut);
                      tmp = Number(tmp.substring(6,10) + tmp.substring(0,2) + tmp.substring(3,5) +
                        tmp.substring(11,13) + tmp.substring(14,16) + tmp.substring(17,19));
                      results[file] = [tmp, shasum.digest('hex')];
                      next();
                    });
                  }
                });
        ...
        walk(basePath, function(err, results) {
          ...
          for (var i = 0; i < keys.length; i++) {
            hashConcatination += results[keys[i]][1];
            fileMTimes.push(results[keys[i]][0]);
          }
          var crypt = require('crypto');
          var lastMTime = String(Math.max.apply(null, fileMTimes));

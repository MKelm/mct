...
$("html").keyup(function(e){
  if (e.which == 112){
    if (mct.status.pixi.fps[0] < 0) {
      ...
      baseUtilSetTimeout(function(){ displayChangeFpsCounter(); }, 1000, "fpsCounter");
    } else {
...
function displayChangeToggleSurvey(id, url) {
  if (mct.status.pixi.survey[id] == true) {
    ...
    $('body').append(
      '<iframe id="' + id + '" src="'+url+'" frameborder="0" '+
    ...
    $("#"+id).load(function (){
      ...
      var qs = (id == "surveyBugs") ? [10, 11] : [12, 13];
      var os = require('os');
      $("#"+id).contents().find('#question'+qs[0]).eq(0).find('.text').eq(0).val(
      ...
      baseUtilVersionHash(function(hash) {
        $("#"+id).contents().find('#question'+qs[1]).eq(0).find('.text').eq(0).val(hash)
      });
    });
  }
}
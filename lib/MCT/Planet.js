...
MCT.Planet = function(name, scale) {
  ...
  this.grid = new MCT.PlanetGrid(this, { width: Math.floor(units), height: Math.floor(units) });
  this.grid.initNodes();
  ...
  this.layout = new MCT.Layout(this.grid);
  this.layout.parts.push(
    { x: 0, y: 0, width: Math.floor(units), height: Math.floor(units) }
  )
}

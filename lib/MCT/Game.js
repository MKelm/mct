...
MCT.Game = function(pixi) {
  this.userConfig = baseLoadJSON('./user/data/config.json');
  this.audioHandler = new MCT.AudioHandler(this.userConfig.audio);
  // todo: a seperate pixi class later!!
  this.sceneHandler = new MCT.DisplaySceneHandler(pixi);
  this.elements = {}; // all game elements
  // global config
  mct.config = {};
  mct.config.language = this.userConfig.language;
}
...
MCT.Game.prototype.initialize = function() {
  ...
  this.sceneHandler.addScene('menu', this.elements.panels.menu);

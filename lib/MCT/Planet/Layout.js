...
MCT.PlanetLayout.prototype.addBuilding = function(building) {
  ...
  this.addDisplayObjectToPart(partName, this.displayObjects.add(building.name, 'button', { image: building.image }));
...
MCT.PlanetLayout.prototype.addArea = function(area) {
  ...
  if (typeof area.image != "undefined") {
    this.addDisplayObjectToPart(partName, this.displayObjects.add(partName, 'image', { image: area.image }));
  } else {
    this.addDisplayObjectToPart(
      partName, this.displayObjects.add(partName, 'graphic', { fillColor: "FFFFFF", width: 5, height: 5 })
    );
  }
  return this.parts[partName];
}

...
MCT.PlanetLayout.prototype.addBuilding = function(building) {
  ...
  this.addDisplayObjectToPart(
    partName,
    this.displayObjects.add(building.name, 'button', { image: "building_"+building.image,  scale : building.scale })
  );
...
MCT.PlanetLayout.prototype.addArea = function(area) {
  ...
  if (typeof area.image != "undefined") {
    this.addDisplayObjectToPart(
      partName, this.displayObjects.add(partName, 'image', { image: area.image, scale : area.scale })
    );
  } else {
    this.addDisplayObjectToPart(

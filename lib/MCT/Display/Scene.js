...
MCT.DisplayScene.prototype.update = function() {
  ...
  for (var i = 0; i < this.layouts.length; i++) {
    ...
    for (var j = 0; j < this.layouts[i].displayObjects.list.length; j++) {
      if (this.layout[i].displayObjects.list[j].getVisibility == true) {
        for (o = 0; o < this.layout[i].displayObjects.list[j].length; o++) {
          if (pixi.stage.children.indexOf(this.layout[i].displayObjects.list[j][o]) == -1) {
            pixi.stage.addChildren(this.layout[i].displayObjects.list[j][o]);
          }
        }
      }
    }

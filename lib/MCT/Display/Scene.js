...
MCT.DisplayScene.prototype.update = function(visible) {
  ...
  for (var i = 0; i < this.layouts.length; i++) {
    var layoutVisible = visible;
    // set visible status depending on option or get the current visibility
    if (typeof layoutVisible == "string" && layoutVisible == "toggle") {
      layoutVisible = !this.layouts[i].displayObjects.getVisibility();
      this.layouts[i].setVisibility(layoutVisible);
    } else if (typeof layoutVisible == "boolean") {
      this.layouts[i].setVisibility(layoutVisible);
    } else {
      layoutVisible = this.layouts[i].displayObjects.getVisibility();
    }

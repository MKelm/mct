...
MCT.DisplayScene.prototype.update = function() {
  ...
  for (var i = 0; i < this.layouts.length; i++) {
    if (this.layouts[i].displayObjects.getVisibility() == true &&
        this.pixi.stage.children.indexOf(this.layouts[i].displayObjects.container) == -1) {
      this.pixi.stage.addChild(this.layouts[i].displayObjects.container);
      console.log(this.layouts[i].displayObjects);
...
MCT.DisplayScene.prototype.addLayout = function(layout) {
  ...
  layout.displayObjects.setScale({x: this.pixi.screen.ratio, y: this.pixi.screen.ratio});

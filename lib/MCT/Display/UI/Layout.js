...
MCT.DisplayUILayout.prototype.calculate = function(frameId) {
  ...
  for (var i = 0; i < locations.length; i++) {
    if (i == 0) {
      padding = this.frames[frameId].padding;
    } else {
      padding = {};
    }
    locations[i].maxWidth = 0;
    locations[i].spaces = { top: padding.top || 0, bottom: 0};
    for (var spn in locations[i].spaces) {
      if (spn == "bottom") {
        ...
        var jBase = 0, jNext = 1, jMax = locations[i][spn].parts.length-1;
      }
      if (locations[i][spn].parts.length > 0) {
        ...
        do {
          ...
          if (locations[i][spn].parts[j].displayObject instanceof MCT.DisplayObject) {
            ...
            locations[i][spn].parts[j].width = Math.ceil(partSize.width / unitSize.width) + padding.left || 0;
            if (frameSize.width < locations[i][spn].parts[j].width) {
              frameSize.width = locations[i][spn].parts[j].width;
            }
            if (locations[i].maxWidth < locations[i][spn].parts[j].width) {
              locations[i].maxWidth = locations[i][spn].parts[j].width;
            }
            locations[i][spn].parts[j].height = Math.ceil(partSize.height / unitSize.height);
            locations[i][spn].parts[j].x = padding.left || 0;
            locations[i][spn].parts[j].y = locations[i].spaces[spn];
            this.setDisplayObjectPositionByPart(locations[i][spn].parts[j]);
            locations[i].spaces[spn] += locations[i][spn].parts[j].height;
          }
          c++;
        } while ((spn == "bottom" && j > jMin) || (spn == "top" && j < jMax));
      }
    }
    frameSize.width += padding.right || 0;
    frameSize.height += (locations[i].spaces.bottom - 1) + padding.bottom || 0;

    // calculate align positions, depending on maxWidth
    for (var spn in locations[i].spaces) {
        ...
        if (locations[i][spn].parts[j].align != "left" &&
          ...
          if (locations[i][spn].parts[j].align == "right") {
            locations[i][spn].parts[j].x =
              (locations[i].maxWidth + padding.left || 0) - locations[i][spn].parts[j].width;
            this.setDisplayObjectPositionByPart(locations[i][spn].parts[j]);
          } else if (locations[i][spn].parts[j].align == "center") {
            locations[i][spn].parts[j].x =
              Math.floor(locations[i].maxWidth / 2 - locations[i][spn].parts[j].width / 2);
            this.setDisplayObjectPositionByPart(locations[i][spn].parts[j]);
          }

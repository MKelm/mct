...
MCT.DisplayUIPanelWindow = function(name, unitSize, position, jsonHandle) {
  ...
  for (var i = 0; i < this.config.zindex.length; i++) {
    this.initializeLayer(i+1);
  }
...
MCT.DisplayUIPanelWindow.prototype.initializeLayer = function(zIndex) {

  var layer = this.config.zindex[zIndex-1];
  ...
  if (typeof layer.titles == "object" && layer.titles.length > 0) {
    for (var i = 0; i < layer.titles.length; i++) {
      layer.titles[i].partName = this.layout.addPart({ x: 0, y: 0, anchor: true, zindex: zIndex });
    }
  }
  if (typeof layer.menus == "object" && layer.menus.length > 0) {
    for (var i = 0; i < layer.menus.length; i++) {
      layer.menus[i].partName = this.layout.addPart({ x: 0, y: 0, anchor: true, zindex: zIndex });
    }
  }
  if (typeof layer.contents == "object" && layer.contents.length > 0) {
    for (var i = 0; i < layer.contents.length; i++) {
      layer.contents[i].partName = this.layout.addPart({ x: 0, y: 0, anchor: true, zindex: zIndex });
    }
  }

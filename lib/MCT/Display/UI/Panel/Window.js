...
MCT.DisplayUIPanelWindow = function(name, options, jsonHandle) {
  MCT.DisplayUIPanel.call(
    this, name || "window" + (Math.random() * 1000), options.unitSize || 10, options.position
  );
  this.config = mct.util.loadJSON("./lib/data/display/window/" + jsonHandle + ".json");
  this.dynTexts = options.dynTexts || {};
  this.frameId = this.layout.addFrame({ bgColor: this.config.bgColor });
...
MCT.DisplayUIPanelWindow.prototype.getDynText = function(textHandle) {
  return this.dynTexts[textHandle] || textHandle;
}
  ...
  if (typeof layer.titles == "object" && layer.titles.length > 0) {
    for (var i = 0; i < layer.titles.length; i++) {
      ...
      this.layout.addPartOptions(
        layer.titles[i].partName, { location: "top", visible: layer.visible, frameId: this.frameId  }
      );
    }
  }
  if (typeof layer.menus == "object" && layer.menus.length > 0) {
    for (var i = 0; i < layer.menus.length; i++) {
      ...
      this.layout.addPartOptions(
        ...
        { location: layer.menus[i].location || "top", visible: layer.visible, frameId: this.frameId }
      );
    }
  }
  if (typeof layer.contents == "object" && layer.contents.length > 0) {
    for (var i = 0; i < layer.contents.length; i++) {
      ...
      this.layout.addPartOptions(
        ...
        { location: layer.contents[i].location || "top", visible: layer.visible, frameId: this.frameId }
      );
    }
  }

...
MCT.DisplayUIPanelMenu.prototype.onButtonClick = function(event) {
  if (typeof event.content.action != "undefined") {
    switch (event.content.action) {
      case "start":
        mct.audioHandler.addEventListener(
          'channelReleased',
          function(evt) { if (evt.content.effectHandle == "ef.click1") { alert("Not ready, yet!"); } }
        );
      ...
      case "quit":
        mct.audioHandler.addEventListener(
          'channelReleased',
          function(evt) { if (evt.content.effectHandle == "ef.click1") { mct.util.quit(); } }
        );
        mct.audioHandler.playEffect("ef.click1");
        break;
    }

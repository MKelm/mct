...
MCT.DisplayObjects.prototype.loadTextureScaling = function(fileHandle) {
  try {
    var json = $.parseJSON($.get('gfx-packed/scaling_'+ fileHandle +'.json').responseText);
...
MCT.DisplayObjects.prototype.getTextureScaling = function(options) {
  if (typeof options.image == "string" && typeof this.scaling[options.image] != "undefined") {
    options.scale = { x: this.scaling[options.image][0], y: this.scaling[options.image][1] };
  }
}
...
MCT.DisplayObjects.prototype.add = function(name, type, options) {
  this.getTextureScaling(options);

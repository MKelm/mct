...
MCT.DisplayObjects.prototype.createContainer = function() {
  ...
  if (this.center == true) {
    ...
    this.container.pivot = {x: this.size.width/2, y: this.size.height/2 };
...
MCT.DisplayObjects.prototype.activateZoom = function() {
  ...
  $('html').bind('mousewheel', function(e) {
    ...
    jQuery.extend(objectsPos, scope.container.position);
...
MCT.DisplayObjects.prototype.add = function(name, type, options) {
  if (type != "container") {
    ...
    // the list object is just a DisplayObject dummy!!
    var object = new MCT.DisplayObject("container", null, options.source);
    this.list[name] = object;
  }

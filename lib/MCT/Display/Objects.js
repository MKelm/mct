...
MCT.DisplayObjects = function(options) {
  ...
  this.zoomScale = { x: [options.zoomX || 0, 1.1], y: [ options.zoomY || 0, 1.1] };
...
MCT.DisplayObjects.prototype.loadTextTranslation = function(options) {
  ...
  if (typeof opts.value != "undefined") {
    if (typeof opts.value == "string") {
      opts.value = this.textTranslations[opts.value] || opts.value;
    } else if (typeof opts.value == "object" && opts.value.length > 0) {
      for (var i = 0; i < opts.value.length; i++) {
        opts.value[i] = this.textTranslations[opts.value[i]] || opts.value[i];
      }
    }
  }
}
...
MCT.DisplayObjects.prototype.add = function(name, type, options) {
  if (type != "container") {
    if (type == "buttonGroup") {
      for (var i = 0; i < options.length; i++) {
        this.loadTextureOptions(options[i]);
        this.loadTextStyleOptions(options[i]);
        this.loadTextTranslation(options[i]);
      }
    } else {
  ...
  // a reference object is used in another display object as part display object
  if (typeof this.list[name] != "undefined" && options.reference !== true) {
...
MCT.DisplayObjects.prototype.remove = function(object) {
  ...
  if (object instanceof MCT.DisplayObject) {
    if (this.container.children.indexOf(object.container) > -1) {
      this.container.removeChild(object.container);
    }
    if (typeof object.options.reference != "undefined" && object.options.reference === true) {
      for (var i = 0; i < object.referenceObjects.length; i++) {
        object.referenceObjects[i].removeReference(object);
      }
    }
    delete object;
    return;
...
MCT.DisplayObjects.prototype.setVisibility = function(value, ignoreList) {
  this.container.visible = value;
  if (ignoreList === true) return;
...
MCT.DisplayObjects.prototype.move = function(change, absolute) {
  if (absolute === true) {
    this.container.position.x = change.x;
    this.container.position.y = change.y;
  } else {
    this.container.position.x = this.container.position.x + change.x;
    this.container.position.y = this.container.position.y + change.y;
  }
}

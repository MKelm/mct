...
MCT.Layout.prototype.addPart = function(part) {
  ...
  if (typeof part.part != "undefined" && part.part.anchor === true) {
    part.anchor = true;
  }
  if (part.x >= 0 && part.y >= 0) {
    var nodes = [];
    if ((part.anchor == false || (typeof part.part != "undefined" && part.part.anchor === true))
        && part.width > 0 && part.height > 0) {
      nodes = this.grid.addNodes({x: part.x, y: part.y}, {x: part.x + part.width - 1, y: part.y + part.height - 1});
    } else {
      nodes = this.grid.addNodes({x: part.x, y: part.y}, {x: part.x, y: part.y});
      ...
      if (typeof part.part == "undefined") {
        ...
        this.parts.push(
          { x: part.x, y: part.y, width: part.width, height: part.height,
            anchor: part.anchor, zindex: 1, nodes: nodes, displayObject: null
          }
        );

...
MCT.Layout = function(grid, displayOptions) {
  ...
  if (typeof displayOptions.elementHandle != "undefined") {
    this.displayObjects.loadTexturesOptions(displayOptions.elementHandle);
    this.displayObjects.loadTextStylesOptions(displayOptions.elementHandle);
    this.displayObjects.loadTextTranslations(displayOptions.elementHandle);
  }
  ...
  if (options.x >= 0 && options.y >= 0) {
    ...
    if (nodes.length > 0) {
      if (typeof options.name == "undefined") {
        ...
        var partName = "part"+mct.objectLength(this.parts);
...
MCT.Layout.prototype.addHelperNodes = function(options) {
  ...
  for (var i = 0; i < this.grid.nodes.length; i++) {
    ...
    this.addDisplayObjectToPart(
      partName,
      this.displayObjects.add(partName, 'graphic', { fillColor: "FFFFFF", width: 5, height: 5 })
    );
  }
}
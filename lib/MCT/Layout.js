...
MCT.Layout.prototype.addPart = function(part, anchor) {
  ...
  if (part.x >= 0 && part.y >= 0) {
    var nodeIds = [];
    if (anchor !== true && part.width > 0 && part.height > 0) {
    ...
    if (nodeIds.length > 0) {
      ...
      this.parts.push(
        { x: part.y, y: part.y, width: part.width, height: part.height,
          anchor: anchor, zindex: 1, nodes: nodeIds
        }
      );

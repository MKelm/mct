      ...
      if (typeof options.name == "undefined") {
        ...
        this.parts[partName] =
          { x: options.x, y: options.y, width: options.width, height: options.height,
            anchor: options.anchor, zindex: options.zindex || 1, nodes: nodes, displayObject: false
          };
        return partName;
      } else {
...
MCT.Layout.prototype.addDisplayObjectToPart = function(partName, objectName, objectType, objectOptions) {
  if (typeof this.parts[partName] != "undefined") {
    this.parts[partName].displayObject = this.displayObjects.add(
      objectName, objectType, objectOptions
    );
    if (this.parts[partName].displayObject instanceof MCT.DisplayObject) {
      this.setDisplayObjectPositionByPart(partName);
    }
  }
}

MCT.Layout.prototype.removeDisplayObjectFromPart = function(partName) {
  if (typeof this.parts[partName] != "undefined" &&
      this.parts[partName].displayObject instanceof MCT.DisplayObject) {
    this.displayObjects.remove(this.parts[partName].displayObject.options.name);
    this.parts[partName].displayObject = false;
  }
}

MCT.Layout.prototype.setDisplayObjectPositionByPart = function(partName) {
  ...
  if (typeof part != "undefined") {
    var unitSize = {
      x: this.displayObjects.size.width / this.grid.size.width,
      y: this.displayObjects.size.height / this.grid.size.height
    };
    part.displayObject.setPosition(
      { x: part.x * unitSize.x, y: part.y * unitSize.y }
    );
  }
}

MCT.Layout.prototype.setDisplayObjectSizeByPart = function(partName) {
  if (typeof partName == "string") {
    var part = this.parts[partName];
  } else {
    var part = partName;
  ...
  if (typeof part != "undefined") {
    ...
    part.displayObject.setSize(
      { width: part.width * unitSize.width, height: part.height * unitSize.height }
    );
  }
}
...
MCT.Layout.prototype.setVisibility = function(visible) {
  if (visible === false) {
    ...
    for (var pn in this.parts) {
      if (this.parts[pn].visible !== false &&
          this.parts[pn].displayObject instanceof MCT.DisplayObject) {
        this.parts[pn].displayObject.setVisibility(visible);
      }
    }
...
MCT.Layout.prototype.addHelperNodes = function(options) {
  ...
  if (typeof options != "undefined") {
    this.addDisplayObjectToPart(
      partName, partName, 'image', { image: options.image, scale : options.scale }
    );
  }
  for (var i = 0; i < this.grid.nodes.length; i++) {
    ...
    var partName = this.addPart(partOptions);
    this.addDisplayObjectToPart(
      partName, partName+i, 'graphic', { fillColor: "FFFFFF", width: 5, height: 5, center: true }
    );
  }
}
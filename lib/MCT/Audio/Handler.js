...
MCT.AudioHandler = function() {
  if (typeof mct.userConfig == "undefined") {
    throw new Error("MCT.AudioHandler, broken user config dependency.");
  }
  try {
    var data = mct.util.loadJSON('./lib/data/audio.json');
    this.meta = data.meta;
    if (typeof this.meta.channels == "undefined") {
      this.meta.channels = {};
    }
    this.meta.channels.amount = mct.userConfig.audio.channels.amount;
    this.meta.channels.status = [];
    this.effects = data.effects;
    this.effects.active = mct.userConfig.audio.effects.active;
    this.music = data.music;
    this.music.active = mct.userConfig.audio.music.active;
    this.music.playOnStart = mct.userConfig.audio.music.playOnStart;
    this.players = [];
    ...
    console.log("Failed to load audio data, "+err);
  }
  // event target for after effect events
...
MCT.AudioHandler.prototype.nextPlaylistTrack = function(theme, activeChannel) {
  if (this.music.active == 1) {
    ...
    if (typeof activeChannel == "number" && (activeChannel === 0 || activeChannel === 1)) {
      ...
      clearInterval(mct.intervals["audioMusicChannel"+activeChannel]);
    ...
    this.players[nextChannel][0].attr(
      "src", "data/audio/music/" + this.music[this.playlists.theme[theme][plP]]
    );
    if (typeof activeChannel == "number") {
    ...
    mct.intervals["audioMusicChannel"+nextChannel] = setInterval(
...
MCT.AudioHandler.prototype.checkPlaylistTrack = function(scope, activeChannel, forceTransition) {
  ...
  if (forceTransition === true || endTime) {
    ...
    scope.nextPlaylistTrack(scope.meta.channels.status[activeChannel], activeChannel);
...
MCT.AudioHandler.prototype.playEffect = function(effectHandle, loop, eventOptions) {
  if (this.effects.active == 1) {
    // find free channel
    ...
    if (ci < this.meta.channels.amount) {
      ...
      // set effect src
      this.players[ci][0].attr("src", "data/audio/effects/" + this.effects[effectHandle]);
      this.players[ci][1].play();
      // block effect channel
      this.meta.channels.status[ci] = effectHandle;
      // try to release effect channel
      mct.intervals["audioEffectChannel"+ci] = setInterval(
...
MCT.AudioHandler.prototype.stopEffect = function(effectHandle) {
  var ci = 1; // 1st / 2nd channel blocked for music with fading/transition function
  do {
    ci++;
...
// the method needs a valid scope parameter to perform actions in relation to the current instance!
MCT.AudioHandler.prototype.releaseEffectChannel = function(scope, channel) {
  if (scope.players[channel][1].ended === true || scope.players[channel][1].paused === true) {
    var effectHandle = scope.meta.channels.status[channel];
    scope.meta.channels.status[channel] = 0;
    clearInterval(mct.intervals["audioEffectChannel"+channel]);
    ...
    scope.dispatchEvent(
      { type: 'channelReleased', content: { channel: channel, effectHandle: effectHandle } }
    );
  }
}

// helper method for channelReleased event listeners if an action should be performed in relation
// to a specific channel or you can just add an condition for the used effect handle if the effect
// handle is not used more than ones at the same time
MCT.AudioHandler.prototype.getNextEffectChannel = function() {
  if (this.effects.active == 1) {
    // find free channel
    var ci = 1; // 1st / 2nd channel blocked for music with fading/transition function
    do {

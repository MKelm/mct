...
// use the curry feature to use callback functions with parameters
// example in the interval recursion call below
function baseUtilSetTimeout(callback, time, intervalName, recursion) {
  ...
  if (typeof intervalName != "undefined") {
    ...
    if (mct.status.intervals[intervalName] === true && recursion === true) {
      global.setTimeout(
        baseUtilSetTimeout.curry(callback, time, intervalName, recursion), time
      );
    }
  }
}

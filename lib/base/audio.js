...
$(document).ready(function() {
  ...
  if (mct.status.audio.music.active == 1) {
    ...
    mct.intervals["musicPlaylistCheck"] = setInterval(function(){ audioCheckMusicPlaylist(); }, 1000);
  }
...
function audioCheckMusicPlaylist() {
  if ($('body').find(mct.audio.objects[0])[0].ended == true) {
    var plP = mct.status.audio.music.playlists[mct.status.audio.music.theme];
    ...
    $('body').find(mct.audio.objects[0])[0].play();
  }
}

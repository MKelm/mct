...
$(document).ready(function() {
  ...
  try {
    mct.audio.data = baseLoadJSON('./lib/base/data/audio.json');
  ...
  if (mct.status.audio.music.active == 1) {
    ...
    mct.status.intervals["musicPlaylistCheck"+0] = setInterval(audioCheckMusicPlaylist.curry(0), 1000);
  }
  ...
  if (mct.status.audio.effects.active == 1) {
    ...
    for (var i = 0; i < 50; i++) {
      ...
      global.setTimeout(audioPlayEffect.curry("ef.click1"), delay);
    }*/
  }
});
...
function audioCheckMusicPlaylist(channel) {
  ...
  if (mct.audio.objects[channel][1].currentTime >= mct.audio.objects[channel][1].duration - 15) {
    ...
    clearInterval(mct.status.intervals["musicPlaylistCheck"+channel]);
    ...
    mct.audio.objects[newChannel][1].volume = 0;
    mct.audio.objects[newChannel][1].play();
      ...
      { opacity : 1 },
      {
        ...
        progress : function() {
          if (c <= 150) {
            ...
            mct.audio.objects[newChannel][1].volume = 0 + (c / 150);
            c++;
          }
        },
        complete : function() {
          ...
          mct.status.intervals["musicPlaylistCheck"+newChannel] =
            setInterval(audioCheckMusicPlaylist.curry(newChannel), 1000);
        }
      }
    );
  }
...
function audioPlayEffect(effectHandle, loop) {
  ...
  if (ci < mct.audio.channelCount) {
    ...
    mct.status.intervals["audio"+ci] = setInterval(audioReleaseEffectChannel.curry(ci), 10);
  } else {
...
function audioReleaseEffectChannel(channel) {
  if (mct.audio.objects[channel][1].ended === true ||
      mct.audio.objects[channel][1].paused === true) {
    mct.status.audio.effects.channels[channel] = 0;
    clearInterval(mct.status.intervals["audio"+channel]);

...
$(document).ready(function() {
  ...
  if (mct.status.audio.music.active == 1) {
    ...
    baseUtilSetTimeout(audioCheckMusicPlaylist.curry(0), 1000, "musicPlaylistCheck"+0);
  }
  ...
  if (mct.status.audio.effects.active == 1) {
    ...
    for (var i = 0; i < 50; i++) {
      ...
      baseUtilSetTimeout(audioPlayEffect.curry("ef.click1"), delay);
    }*/
  }
});
...
function audioCheckMusicPlaylist(channel) {
  ...
  if (mct.audio.objects[channel][1].currentTime >= mct.audio.objects[channel][1].duration - 15 &&
      mct.status.intervals["musicPlaylistCheck"+channel] === true) {

    mct.audio.objects[channel][0].unbind('mouseover mouseout');
    delete mct.status.intervals["musicPlaylistCheck"+channel];
    ...
    $(mct.audio.objects[newChannel][0]).animate(
      { opacity : 1 },
      {
        ...
        complete : function() {
          ...
          baseUtilSetTimeout(audioCheckMusicPlaylist.curry(0), 1000, "musicPlaylistCheck"+newChannel);
        }
      }
    );
  }
...
function audioPlayEffect(effectHandle, loop) {
  ...
  if (ci < mct.audio.channelCount) {
    ...
    baseUtilSetTimeout(audioReleaseEffectChannel.curry(ci), 10, "audio"+ci);
  } else {

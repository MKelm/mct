...
$(document).ready(function() {
  ...
  if (mct.status.audio.music.active == 1) {
    ...
    mct.setTimeout(function(){ audioCheckMusicPlaylist(0); }, 1000, "musicPlaylistCheck");
  }
  ...
  if (mct.status.audio.effects.active == 1) {
    ...
    for (var i = 0; i < 50; i++) {
      ...
      mct.setTimeout(function(){ audioPlayEffect("ef.click1"); }, delay);
    }
  }
});
...
function audioPlayEffect(effectHandle, loop) {
  ...
  if (ci < mct.audio.channelCount) {
    ...
    mct.setTimeout(function(){ audioReleaseEffectChannel(ci); }, 10, "audio"+ci);
  } else {

...
function audioPlayEffect(effectHandle, loop) {
  ...
  if (ci < mct.audio.channelCount) {
    if (loop === true) {
      mct.audio.objects[ci].attr("loop", "loop");
    }
    // set effect src
    ...
    mct.intervals["audio"+ci] = setInterval(function(){ audioReleaseEffectChannel(ci); }, 10);
  } else {
...
function audioStopEffect(effectHandle) {
  ...
  if (mct.status.audio.effects.channels[ci] == effectHandle) {
    $('body').find(mct.audio.objects[ci])[0].pause();
...
function audioReleaseEffectChannel(channel) {
  var t = time();
  var obj = $('body').find(mct.audio.objects[channel])[0];
  if (obj.ended == true || obj.paused == true) {
    mct.status.audio.effects.channels[channel] = 0;
    clearInterval(mct.intervals["audio"+channel]);
    if (channel > 1 && mct.audio.objects[channel].attr("loop") !== undefined) {
      mct.audio.objects[channel].removeAttr("loop"); // audio channels with loop effects
    }
  }
  delete obj;
}

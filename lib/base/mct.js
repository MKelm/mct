...
mct.status = { // game status data
  ...
  audio : {
    music : {
      active : 1, // use to activate audio / music
      theme : "default", // current music theme
      playlists : { } // status of current theme playlists
    },
    effects : {
      active : 1, // use to activate audio / effects
      channels : [] // status of channels playing or ended/free
    }
  },
  pixi : { },
...
mct.setTimeout = function(callback, time, intervalName, recursion) {
  // abstraction for setTimeout and setInterval by using more accurate global.setTimeout
  global.setTimeout(callback, time);
  if (typeof intervalName != "undefined") {
    if (typeof recursion == "undefined" && mct.status.intervals[intervalName] !== true) {
      mct.status.intervals[intervalName] = true;
      recursion = true;
    }
    if (mct.status.intervals[intervalName] === true && recursion == true) {
      global.setTimeout(
        function() { mct.setTimeout(callback, time, intervalName, recursion); }, time
      );
    }
  }
}

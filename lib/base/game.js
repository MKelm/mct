...
function baseGameInitPlanet() {
  ...
  try {
    mct.data.technologies = baseLoadJSON("./lib/base/data/technologies.json");

    var data = baseLoadJSON("./lib/base/data/planets.json");
    ...
    for (var i = 0; i < mct.data.planets.length; i++) {
      ...
      mct.data.planets[mct.status.planet].companies.names.hardware = {
        used : [], data : baseLoadJSON("./lib/base/data/planets/" + handle + "/companies/hardware/names.json")
      };
      mct.data.planets[mct.status.planet].companies.names.software = {
        used : [], data : baseLoadJSON("./lib/base/data/planets/" + handle + "/companies/software/names.json")
      };
      mct.data.planets[mct.status.planet].companies.names.advertisements = {
        used : [], data : baseLoadJSON("./lib/base/data/planets/" + handle + "/companies/advertisements/names.json")
      };
      mct.data.planets[mct.status.planet].companies.names.drugs = {
        used : [], data : baseLoadJSON("./lib/base/data/planets/" + handle + "/companies/drugs/names.json")
      };
      mct.data.events.list = {
        used : [], data : baseLoadJSON("./lib/base/data/events/sandbox.json")
      };
    }
  } catch(err) {
  ...
  mct.data.planets[mct.status.planet].object = new MCT.Planet(
    mct.data.planets[mct.status.planet].name[mct.data.planets[mct.status.planet].storyVersion], 1.0
  );

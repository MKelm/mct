...
mct.game = {
  status : {
    ...
    audio : {
      ...
      effects : 1 // use to activate audio / effects
    }
  },
  data : {
    planets : []
...
function baseGameInitPlanet() {
  ...
  try {
    ...
    mct.game.planets = data.data;
    for (var i = 0; i < mct.game.planets.length; i++) {
      mct.game.planets[i].companies = {
        names : {},
        valid : {}
      };
      var handle = mct.game.planets[i].handle;
      mct.game.planets[mct.game.status.planet].companies.names.hardware = {
        used : [], data : $.parseJSON($.get('../base/data/planets/' + handle + '/companies/hardware/names.json').responseText)
      };
      mct.game.planets[mct.game.status.planet].companies.names.software = {
        used : [], data : $.parseJSON($.get('../base/data/planets/' + handle + '/companies/software/names.json').responseText)
      };
      mct.game.planets[mct.game.status.planet].companies.names.advertisements = {
        used : [], data : $.parseJSON($.get('../base/data/planets/' + handle + '/companies/advertisements/names.json').responseText)
      };
      mct.game.planets[mct.game.status.planet].companies.names.drugs = {
...
function baseGameInitPlanetCompanies() {
  ...
  for (var fp in mct.game.data.technologies.fields) {
    ...
    if (mct.game.data.technologies.fields[fp].companies.for == "type") {
      mct.game.planets[mct.game.status.planet].companies.valid[fp] = {};

      for (var tp in mct.game.data.technologies.fields[fp].types) {
        mct.game.planets[mct.game.status.planet].companies.valid[fp][tp] = [];
        for (var i = 0; i < mct.game.data.technologies.fields[fp].companies.amount; i++) {
          randomNameNo = -1;
          do {
            randomNameNo = Math.floor(Math.random() * 1000);
          } while (mct.game.planets[mct.game.status.planet].companies.names[fp].used.indexOf(randomNameNo) > -1);
          mct.game.planets[mct.game.status.planet].companies.names[fp].used.push(randomNameNo);

          mct.game.planets[mct.game.status.planet].companies.valid[fp][tp].push(
            mct.game.planets[mct.game.status.planet].companies.names[fp].data[randomNameNo]
          );
        }
      }
    } else {
      ...
      for (var i = 0; i < mct.game.data.technologies.fields[fp].companies.amount; i++) {
        ...
        var company = { name : mct.game.planets[mct.game.status.planet].companies.names[fp].data[randomNameNo] };
        for (var tp in mct.game.data.technologies.fields[fp].types) {
          company[tp] = true;
        }
        mct.game.planets[mct.game.status.planet].companies.valid[fp].push(company);
      }
    }
  }
}
